...
from robust_exp.engine import evaluate, trades_train_fixed_lr_one_epoch
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Publication baseline: TRADES")
    parser.add_argument("--data-root", default="data")
    parser.add_argument("--output-root", default="outputs_publication")
    parser.add_argument("--run-name", default="trades_preact_seed17")
    parser.add_argument(
...
def main() -> None:
    ...
    write_json(
        run_dir / "config.json",
        {
            **vars(args),
            ...
            "method": "TRADES",
            "implementation_note": "TRADES objective with the shared publication schedule/backbone",
        },
    )
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for group in optimizer.param_groups:
            group["lr"] = lr
        metrics = trades_train_fixed_lr_one_epoch(
            model,
            train_loader,
            optimizer,
            device,
        ...
        print(
            f"epoch={epoch:03d} lr={lr:.4g} loss={metrics['train_trades_loss']:.4f} "
